...
module.exports = observableModule.fromObject({
  //========== Properties ==========//
  usernameInput: "test",
  emailInput: "test@register.com",
  passwordInput: "password",
  passwordReentryInput: "password",

  inputFields: [],
  verifyEmailInputFields: [], // 6 digit email verification code input
  ...
  confirmEmailSectionsOpacity: 0, // Affects success message & verification code input sections
  //========== Functions ==========//
  unfocusInput(){
  ...
  onVerifyEmailInputChangeHandler(args){
    if(args.value.length == 1){
      for(var i=0; i<5; i++){
        if(this.verifyEmailInputFields[i] == args.object){
          setTimeout(()=>{
            this.verifyEmailInputFields[i+1].focus();
          },5);
          return;
        }
      }
  ...
  onLoaded(args){
    ...
    this.inputFields.push(args.object.getViewById("emailField"));
    ...
    for(var i=0; i<6; i++){
      this.verifyEmailInputFields.push(args.object.getViewById("verifyEmailInput"+i.toString()));
      console.log(this.verifyEmailInputFields[i]);
      this.verifyEmailInputFields[i].on("textChange", this.onVerifyEmailInputChangeHandler, this);
    }
  ...
  onTick(args){
    if(this.sentConfirmationEmail){
      // Once verification email sent, set email verification sections to fade in
      if(this.confirmEmailSectionsOpacity == 1) return;
      this.confirmEmailSectionsOpacity = Math.min(1, this.confirmEmailSectionsOpacity
        + (args.deltaTime/ 500));
    }
  },
