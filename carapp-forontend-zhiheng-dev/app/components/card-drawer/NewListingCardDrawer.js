...
class NewListingCardDrawer extends cardViewModule.CardView{
  /* NOTE:
  ...
  */
  ...
  //========== Functions ==========//
  setToOpen(){
    this.getViewById('cardDrawerScrollView')
      .className = "listCarCardDrawerScrollView";
    this.animate({
      translate: {
        x: 0,
        y: 0,
  ...
  setToSuspended(){
    // Change class of scrollview to add more margin at the top of the scrollview
    // so that user cannot see the image when drawer is suspended
    this.getViewById('cardDrawerScrollView')
      .className = "listCarCardDrawerScrollView suspended";
  ...
  setToSuspendedNoAnimation(){
    ...
    if(appSettings.getBoolean('iPhoneX')){
      this.translateY = this.height-238;
    } else { // Other phones
      this.translateY = this.height-182;
    }
  ...
  onPan(args){
    ...
    let scrollView = view.getViewById('cardDrawerScrollView');
    // Check whether the scrollview within the CardDrawer is scrolled to the top
    if(scrollView.verticalOffset == 0){
      switch(args.state){
        ...
        case 3://Pan end
          ...
          console.log(drawerPositionRatio);
...
var properties = {
  ...
  drawerClosed: new propertiesModule.Property({
    name: 'drawerClosed',
    defaultValue: ()=>{},
  }),
...
//========== Initialize ==========//
for(var key in properties) properties[key].register(NewListingCardDrawer);
module.exports.NewListingCardDrawer = NewListingCardDrawer;
